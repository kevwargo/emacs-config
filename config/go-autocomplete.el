...
(defcustom ac-go-gocode-port nil
  "Custom tcp port to listen to.
If nil, default gocode's server socktype and address are used"
  :type 'integer
  :group 'go-autocomplete)
...
(defun ac-go-invoke-autocomplete ()
  (let ((temp-buffer (generate-new-buffer "*gocode*")))
    (unwind-protect
        (let ((args (append (if ac-go-gocode-port
                                (list
                                 "-sock"
                                 "tcp"
                                 "-addr"
                                 (format "127.0.0.1:%d" ac-go-gocode-port)))
                            (list "-f=emacs"
                                  "autocomplete"
                                  (or (buffer-file-name) "")
