...
(defun magit-custom-keys-hook ()
  (keymap-local-set "C-<tab>" 'magit-section-cycle-diffs)
  (keymap-local-set "w" 'magit-diff-toggle-whitespace))
...
(defun magit-status-custom-keys-hook ()
  (keymap-local-set "K" 'magit-delete-file))

(defun magit-stashes-custom-keys-hook ()
  (keymap-local-set "p" 'magit-stash-pop))
